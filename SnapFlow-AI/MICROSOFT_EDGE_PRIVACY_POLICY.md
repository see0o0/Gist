# SnapFlow Privacy Policy for Microsoft Edge
...
SnapFlow is a screenshot and web-capture extension for Microsoft Edge. It lets users capture a full webpage, the currently visible browser area, or a user-selected area, and manage the resulting screenshots locally on their device.
...
SnapFlow's single purpose is to let users capture webpages in Microsoft Edge and manage the resulting screenshots locally on their device.
...
| Selected interface language | To keep the user's language choice | Locally in extension storage |
| Temporary capture-task information, including a job ID, capture type, tab/window identifiers, page URL/title, task status, and timestamps | To coordinate an active screenshot operation | Temporarily in extension session storage and removed after the task is completed or cleared |
| Analytics consent and a random pseudonymous installation ID | To remember whether the user opted in and associate anonymous analytics data from the same installation | Locally in extension storage; the identifier is sent to Mixpanel only after opt-in |
...
SnapFlow also reads the Microsoft Edge interface language locally to choose an initial interface language when the user has not made a manual selection.
...
- Temporary capture-task information is cleared after the task is completed, cancelled, fails, or is identified as stale. Session storage is also managed by Microsoft Edge.
- SnapFlow V1.0 does not transmit these records to the developer or to a SnapFlow server.
- SnapFlow does not use browser synchronization for screenshot data.
- When the user explicitly chooses Copy or Download, the requested image is written to the device clipboard or download location through Microsoft Edge features. This is initiated and controlled by the user and is not a transfer to SnapFlow or the developer.
- Analytics consent and the random installation ID remain in local extension storage until the user disables analytics, clears extension data, or uninstalls SnapFlow.
...
- remove all SnapFlow extension data by clearing the extension's local data through Microsoft Edge settings or uninstalling SnapFlow.
...
SnapFlow minimizes permissions and keeps screenshot processing within the Microsoft Edge extension environment. It does not transmit screenshot data to a developer-controlled server. Optional analytics data is transmitted to Mixpanel over HTTPS only after consent.

Users should protect access to their device and Microsoft Edge profile. No method of local storage can be guaranteed to be completely secure against someone who already has access to the user's device or browser profile.
